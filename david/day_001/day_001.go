...
// sum first and last ints in each line of txt file, includinng ints in strings
func sumFirstLastInts(content string) int {
	...
	for _, line := range contentArray {
		...
		temp += fmt.Sprint(getSingleInt(first, "first")) // get single integeer from larger possible number
